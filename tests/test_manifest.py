...
class TestScan:
    ...
    def test_retag_updates_new_fields_only(self, tmp_path, make_wav):
        ...
        meta = song / "META.txt"
        meta.write_text("SINGER:emma hesters\nGENRE:pop\n", encoding="utf-8")
        manifest, _ = scan_directory(root, subpath="RAW")
        rec = manifest.records[0]
        rec.meta.singer = "emma heesters"  # the hand-fix
        manifest.upsert(rec)
        manifest.save()
        ...
        assert rec2.meta.singer == "emma heesters"  # NOT clobbered back to the typo
    ...
    def test_import_stems_marks_separated_and_dedupes(self, tmp_path, make_wav):
        ...
        root = tmp_path / "dr"
        # existing record: a TEST_RUN-style full mix whose folder carries a yt id
        ...
        manifest, new, skipped = import_stem_folders(
            root, subpath="RAW/Full", language="en", gender="F")
        manifest.save()
        ...
        _, new2, _ = import_stem_folders(root, subpath="RAW/Full")
        assert new2 == []
