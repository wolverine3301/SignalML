# -*- coding: utf-8 -*-
"""Example driver for the masking track (legacy general-signals experiments, Q12).

Demonstrates the audio-profile system (Q11): parameters come from configs/audio.yaml,
never literals. Becomes a documented example once the real CLI stages land.
"""
...
from signalml.audio.io import save_npz_arrays
from signalml.config import active_profile
from signalml.pipeline.features import mels_from_chunks
from signalml.tasks.masking import build_class_chunks, flatten_class_dict, generate_masking_dataset
...
def main():
    ...
    music = build_class_chunks(
        raw / "instramentalMusic", 1.0, audio_cfg, pitch_augment=True, rng=rng
    )
    ...
    voice_vehicle = generate_masking_dataset(voice, vehicle, label="voiceVehicle", sr=sr, rng=rng)
    ...
    voice_music = generate_masking_dataset(voice, music, label="voiceMusic", sr=sr, rng=rng)
    ...
    sing_vehicle = generate_masking_dataset(singing, vehicle, label="singVehicle", sr=sr, rng=rng)
    ...
    sing_music = generate_masking_dataset(singing, music, label="singMusic", sr=sr, rng=rng)
